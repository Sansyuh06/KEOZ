"""Policy-aware negotiator with LLM offer parser and deterministic bounds clamp."""
...
class PolicyAwareNegotiator:
    ...
    "LLM parses and proposes, deterministic code disposes."
    """
    ...
    def negotiate(self, request: BuyerRequest) -> NegotiationResult:
        ...
        if request.raw_text:
            ...
            if extracted.get("product_id"):
                request.product_id = extracted["product_id"]
            if extracted.get("proposed_price_inr") is not None:
        ...
        if proposed_price >= list_price:
            ...
            # Buyer proposed BELOW floor price (e.g. ₹4,200 < floor ₹4,500)
            # 1. Deterministic Clamp to the higher of floor_price or discount ceiling price
            ...
            # 2. Strategic Privacy Buffer: Counter at effective_floor + buffer
            final_price = BoundsClamp.apply_privacy_buffer(max(raw_clamped_price, effective_floor), effective_floor)
            # Ensure price doesn't exceed list price
