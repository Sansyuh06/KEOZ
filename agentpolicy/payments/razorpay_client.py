"""Razorpay integration client (supporting both live test-mode API keys and deterministic simulator)."""
...
class RazorpayClient:
    ...
    def __init__(self, key_id: Optional[str] = None, key_secret: Optional[str] = None):
        self.key_id = key_id or os.environ.get("RAZORPAY_KEY_ID", "rzp_test_agentpolicy_2026")
    ...
    def create_order(
        ...
        if self.is_live:
            try:
                ...
                # If network or real API error occurs, log and fall back with error metadata
    ...
    def capture_payment(self, payment_id: str, amount_inr: int, currency: str = "INR") -> Dict[str, Any]:
        ...
        - When real test keys are configured, executes POST /v1/payments/{id}/capture.
        - When keys are absent, falls back to simulated captured entity.
        """
        if self.is_live:
            try:
                response = requests.post(
    ...
    def capture_payment_mock(self, order_id: str, amount_inr: int) -> Dict[str, Any]:
        ...
        Note: Used in headless test suites and offline demos to simulate incoming Razorpay webhook events.
        """
