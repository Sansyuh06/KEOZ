"""CLI entry point for KEOZ — The Merchant Command Center for Agentic Commerce."""
...
def _resolve_policy_file(path_str: str = None) -> Path:
    """Find policy file looking at provided path, local keoz.yaml, or examples/merchant_acme.yaml."""
    if path_str and Path(path_str).exists():
        return Path(path_str)
    if Path("keoz.yaml").exists():
        return Path("keoz.yaml")
    if Path("agentpolicy.yaml").exists():
        return Path("agentpolicy.yaml")
    if Path("examples/merchant_acme.yaml").exists():
        return Path("examples/merchant_acme.yaml")
    if Path("examples/agentpolicy.yaml").exists():
        return Path("examples/agentpolicy.yaml")
    return Path(path_str or "keoz.yaml")


def cmd_init(args):
    target = Path(args.output or "keoz.yaml")
    if target.exists() and not args.force:
        console.print(f"[bold yellow]File {target} already exists. Use --force to overwrite.[/bold yellow]")
        return
    with open(target, "w", encoding="utf-8") as f:
        f.write(SAMPLE_YAML)
    console.print(Panel(f"[bold green]Created {target}[/bold green]\nReady to configure your merchant boundaries.", title="KEOZ Init"))
...
def cmd_compile(args):
    ...
    try:
        ...
        console.print(Panel(
            ...
            f"  * x402_config.json\n"
            f"  * openapi.json",
            title="[bold green]KEOZ Policy Compiled Successfully[/bold green]"
        ))
    except Exception as e:
...
def cmd_serve(args):
    ...
    console.print(Panel(
        f"[bold green]Starting KEOZ Merchant Command Center...[/bold green]\n"
        f"* Policy Source: {filepath if filepath.exists() else 'Default Multi-Merchant Registry'}\n"
...
def main():
    parser = argparse.ArgumentParser(description="KEOZ CLI — The Merchant Command Center for Agentic Commerce")
    subparsers = parser.add_subparsers(dest="command", required=True)

    # init
    p_init = subparsers.add_parser("init", help="Initialize keoz.yaml")
    p_init.add_argument("-o", "--output", default="keoz.yaml")
    ...
    # serve
    p_serve = subparsers.add_parser("serve", help="Launch live KEOZ Command Center server")
