...
[![Python](https://img.shields.io/badge/python-3.10%2B-38bdf8.svg)](https://python.org)
[![FastAPI](https://img.shields.io/badge/FastAPI-0.100%2B-06b6d4.svg)](https://fastapi.tiangolo.com)
...
```
Buyer Bot Proposal:
  Product: Pro Annual (List Price: ₹50,000 | COGS: ₹28,350)
  Quantity: 50 seats
  Proposed Price: ₹42,000/seat (16% discount)
  Payment Terms: Net-30 credit
```

| Without KEOZ | With KEOZ |
|---|---|
| ❌ Either hard-rejected (lost ₹21L sale) or blindly accepted by an LLM bot below the merchant's 37% margin floor. | ✅ **1. Deterministic Clamp**: Counters at **₹46,500/seat** (applies 8% volume discount cap + 3% strategic privacy buffer to hide the exact floor).<br>✅ **2. Credit Check**: Net-30 payment terms trigger an **HTTP 202 escalation** for human sign-off.<br>✅ **3. 1-Click Approval & Settlement**: Merchant approves in dashboard &rarr; Razorpay order generated & settled. |
...
│    ├─ Layer 3: Composed Margin Validator (COGS + terms)     │
...
- `acme-saas`: Strict B2B policy (37% margin floor, ₹5L autonomous limit, Pro Annual ₹45k floor).
- `bigco-enterprise`: High-volume policy (30% margin floor, ₹20L autonomous limit, 15% discount cap).
...
- An internal deterministic regex engine handles standard Indian currency formats (`₹42,000`, `1.8L`, `42k`, `Net-30`) with zero external API dependencies.
...
authorization:
  max_autonomous_inr: 500000        # Orders > ₹5L require human sign-off
...
|---|---|---|---|
| `deep_discount` | 80% discount demand (₹9,000 for ₹45k item) | Parameter clamp + privacy buffer | **Clamped to ₹46,500** |
...
| `overspend` | ₹10L order on ₹5L autonomous ceiling | Autonomous spend limit check | **Escalated (HTTP 202)** |
